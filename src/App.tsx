...
socket.on("update_csgo", (_csgo: any, damage?: RoundDamage[]) => {
...
> {
    ...
    verifyPlayers = async (game: CSGO) => {
        ...
        if (
            steamids.every((steamid) => this.state.steamids.includes(steamid))
        ) {
            return;
        }
        ...
        const lacking = notCheckedPlayers.filter((steamid) =>
