        ...
        grenades
            ...
            .reduce(
                (prev, next) => ({
                    ...next,
                    ammo_reserve:
                        (prev.ammo_reserve || 0) + (next.ammo_reserve || 0),
                }),
                { name: "", ammo_reserve: 0 }
            ).ammo_reserve || 0
    );
...
class GrenadeContainer extends React.PureComponent<{
    grenade: string;
    amount: number;
}> {
    render() {
...
export default class SideBox extends React.Component<Props> {
    render() {
        ...
        return (
            <div
                className={`utilitybox ${this.props.side || ""} ${
            ...
            >
                <div className="title_container">
                    ...
                    <div
                        className="subtitle"
                        style={{ color: utilityColor(total) }}
                    >
                        {utilityState(total)}
                    </div>
                </div>
                <div className="grenades_container">
                    <GrenadeContainer
                        ...
                        amount={grenades.smokes}
                    />
                    <GrenadeContainer
                        grenade={
                            this.props.side === "CT" ? "incgrenade" : "molotov"
                        }
                        amount={grenades.inc}
                    />
                    <GrenadeContainer
                        grenade="flashbang"
                        amount={grenades.flashes}
                    />
                    <GrenadeContainer
                        grenade="hegrenade"
                        amount={grenades.hg}
